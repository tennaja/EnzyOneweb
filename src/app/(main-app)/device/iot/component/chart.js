...
export default function Chart({ deviceParameterid,deviceTypeId}) {
  ...
  useEffect(() => {
    if (deviceParameterid != 0 && deviceTypeId != 0) {
      GetGraph(deviceParameterid, formatDate(new Date()), formatDate(new Date()));
  ...
  async function GetGraph(deviceParameterid, dateFrom, dateTo) {
    ...
    if (res.status === 200) {
        ...
        for (let j = 0; j < res.data[0].data.length; j++) {
          ...
          console.log(res.data[0].data[j].time)
        }
        setListLabel(label);
        console.log(label);
          ...
          <select
              ...
              {deviceparameterList.length > 0 &&
                deviceparameterList.map((item) => {
                  ...
                  return (
                  <option className="rounded-lg" key={item.id}>{item.name}</option>)
                })}
            </select>
          <button
